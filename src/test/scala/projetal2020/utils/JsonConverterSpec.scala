...
class JsonConverterSpec extends AnyFunSuite {
  test("convertToJson should convert Position to Json") {
    ...
    assert(JsonConverter.convertToJson(position).toString == expected)
  }

  test("convertToJson should convert Lawn to Json") {
    val lawn = new Lawn(10, 2)
    val expected = """{"x":10,"y":2}"""
    assert(JsonConverter.convertToJson(lawn).toString == expected)
  }
  ...
  test("convertToJson should convert Mower to Json") {
    ...
    assert(JsonConverter.convertToJson(mower).toString == expected)
  }
  ...
  test("convertToJson should convert Settings to Json") {
    val settings = new State(
      ...
      Array(
        ...
        new Mower(Position(1, 2, "E"), Position(5, 1, "E"), "AADAADADDA")
      )
    )
    val expected =
      """{
    ...
    assert(JsonConverter.convertToJson(settings).toString == expected)
  }
}
